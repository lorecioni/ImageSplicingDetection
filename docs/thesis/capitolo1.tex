...
where $\omega$ is the visible light spectrum, $\beta$ is the light wavelength, $e(\beta, x)$ is the illuminant spectrum, $s(\beta, x)$ is the surface reflectance of an object and $c(\beta)$ is the color sensitivity of the camera for each channel.
...
The Fig. \ref{fig:tiago_method_full} shows the above described method pipeline.
